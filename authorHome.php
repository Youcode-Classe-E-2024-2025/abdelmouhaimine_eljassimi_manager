...
<section class="p-6 m-4 rounded-lg">
    <div class="flex items-center justify-center space-x-4">
        <input type="text" placeholder="Type book name or author"
               class="w-1/2 p-2 h-16 shadow-md border rounded-md focus:ring-2 focus:ring-indigo-500 focus:outline-none"/>
...
<section class="m-4 p-6 rounded-lg">
    <div class="flex justify-between">
        ...
        <button id="openModal" class="bg-[#7E55E7] text-white font-bold p-2 w-32 rounded-sm flex justify-center align-center hover:bg-[#5ce1e6] transition">ADD BOOK</button>
    </div>
    <div class="flex justify-around overflow-x-auto space-x-4 mt-4">
        <?php
        $query = "SELECT b.title, b.description, b.rating, b.isbn, b.cover_url, a.name AS author_name
                  FROM books b
                  JOIN book_authors ba ON b.id = ba.book_id
                  JOIN actors a ON ba.author_id = a.id
                  WHERE a.id = $id";
        $result = mysqli_query($connection, $query) or die(mysqli_error($connection));
        $count = 0;
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <div class="min-w-[150px] bg-white flex flex-col gap-4 justify-center p-4 border rounded-md text-center shadow hover:shadow-lg transition">
                <img src="<?php echo $row['cover_url']; ?>" class="w-36 flex justify-center ml-2" alt="Book"/>
                <h3 class="font-semibold text-gray-700 w-40 flex justify-center flex-wrap"><?php echo $row['title']; ?></h3>
                <i class="text-sm font-bold text-gray-500"><?php echo $row['author_name']; ?></i>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <tr class="hover:bg-gray-50 h-20 rounded-lg border-b">
                <td class="py-2 px-4 flex gap-4">
                    <img src="<?php echo $row['cover_url']; ?>" class="w-10" alt="Book Cover">
                ...
                <td class="py-2 px-4 text-indigo-600 cursor-pointer">
                    <button class="bg-[#e9e3fc] px-10 py-3 font-bold">Take book</button>
                </td>
            </tr>
            <?php
        }
    ...
    <div class="bg-white w-96 rounded-lg shadow-lg p-6">
        ...
        <form id="addPackageForm" class="space-y-4" action="addBook.php" method="POST">
            ...
            <div>
                ...
                <select id="AuthorName" name="AuthorName[]" class="w-full text-black focus:outline-none focus:ring focus:ring-[#7E55E7]" multiple size="5">
                    <?php
            ...
            <div>
                ...
                <textarea id="description" name="description" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-[#7E55E7]"></textarea>
            </div>
            <div>
